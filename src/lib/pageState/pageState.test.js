...
test('Page State Manager Tests - Lookup', () => {
    ...
    for(let i = 0; i < n; i++){
        ...
        expect(PageManager.isHomePage(PageManager.pageArray[i].name)).toBe(i===0?true:false);
        expect(PageManager.isEndPage(PageManager.pageArray[i].name)).toBe(i===n-1?true:false);
